...
var qa_dsn = 'https://trycatch-end.qa.91jkys.com/errors/';
...
function track(actionType, param) {
    var data = __assign({ actionType: actionType }, param);
    sendTrackData(data);
    return data;
}
...
var localstoragePrefix = 'mitojs';
var LocalStorageSessionIdKey = localstoragePrefix + ":sessionId";
var LocalStorageChannelKey = localstoragePrefix + ":channel";
function getSessionId() {
    var result = wx.getStorageSync(LocalStorageSessionIdKey);
    if (!result) {
    ...
    wx.removeStorageSync(LocalStorageSessionIdKey);
    return '';
}
...
function getChannel() {
    var result = wx.getStorageSync(LocalStorageChannelKey);
    if (result && result.expireTime > getTimestamp()) {
        setChannel(result.value);
        return result.value;
    }
    wx.removeStorageSync(LocalStorageChannelKey);
    return '';
}
...
function parseQuery(query) {
    ...
    var telescopeString = query['telescope-info'];
    var telescope;
    try {
        telescope = JSON.parse(telescopeString);
    }
    catch (error) {
        console.log('parse query telescope-info failed');
        return;
    }
    if (telescope.sessionId) {
        setSessionId(telescope.sessionId);
    }
    if (telescope.channel) {
        setChannel(telescope.channel);
    }
}
...
var includeHttpUrlTraceIdRegExp = /apigw/;
var maxBreadcrumbs = 20;
var enableTraceId = true;
function manualInit(callback, qa) {
    if (qa === void 0) { qa = false; }
    var isQa = wx.getAccountInfoSync().miniProgram.envVersion !== 'release';
    var envInit = qa ? isQa : !isQa;
    if (envInit) {
        callback();
    }
}
var miniTrackerId = 'mito-uuid';
function backTrackerId() {
    var uuid = wx.getStorageSync(miniTrackerId);
    if (!isEmpty(uuid))
        return uuid;
    uuid = generateUUID();
...
function wxBeforeDataReport(data, customTag) {
    var reportData = data.data;
    if (isReportDataType(reportData)) {
        if (!reportData.customTag && customTag) {
            reportData.customTag = customTag;
        }
        return data;
    }
    return reportData;
}
function myTrack(actionType, param) {
    return track(actionType, __assign(__assign({ id: generateUUID() }, param), getConventionalData()));
}
function appOnLaunch(options) {
    console.log('sdk AppOnLaunch options', options);
    parseQuery(options.query);
}
function appOnShow(options) {
    console.log('sdk apponshow options', options);
}
var startTime = null;
function pageOnShow(page) {
    startTime = getTimestamp();
    var seKeyword = page.options.seKeyword;
    seKeyword && (trackData.seKeywordGlobal = seKeyword);
    console.log('sdk page show', page);
    if (trackData.curTrackPage)
        return;
    trackData.curTrackPage = trackPage({
        trackId: trackData.trackConfig.projectId + "-" + page.route
    });
}
function pageOnHide(page) {
    var endTime = getTimestamp();
    var durationTime = endTime - startTime;
    myTrack(EActionType.DURATION, __assign({ durationTime: durationTime }, trackData.curTrackPage));
    console.log('sdk page hide', page);
    trackData.preTrackPage = trackData.curTrackPage;
    trackData.curTrackPage = null;
}
function getLocalStorageParam() {
...
function getConventionalData() {
    var _a, _b, _c, _d;
    var localStorageData = getLocalStorageParam();
    return __assign(__assign({}, localStorageData), { pageId: (_a = trackData.curTrackPage) === null || _a === void 0 ? void 0 : _a.trackId, referrerId: (_b = trackData.preTrackPage) === null || _b === void 0 ? void 0 : _b.id, referrerUrl: '', sourceEventId: (_c = trackData.preTrackEvent) === null || _c === void 0 ? void 0 : _c.id, appType: '小程序', projectId: (_d = trackData.trackConfig) === null || _d === void 0 ? void 0 : _d.projectId });
}
function trackPage(params) {
    trackData.curTrackPage = myTrack(EActionType.PAGE, params);
    return trackData.curTrackPage;
}
function trackEvent(params) {
    trackData.preTrackEvent = params;
    var result = __assign(__assign({}, params), { pageId: trackData.curTrackPage.trackId, seKeyword: trackData.seKeywordGlobal });
    myTrack(EActionType.EVENT, result);
}

function init(_a) {
    var apikey = _a.apikey, customTag = _a.customTag, projectId = _a.projectId;
    if (apikey) {
        manualInit(function () {
            init$1({
                apikey: apikey,
                ...
                dsn: qa_dsn + "/upload",
                silentConsole: true,
                includeHttpUrlTraceIdRegExp: includeHttpUrlTraceIdRegExp,
                maxBreadcrumbs: maxBreadcrumbs,
                enableTraceId: enableTraceId,
                beforePushBreadcrumb: function (breadcrumb, cruBreadcrumbData) {
                    return beforePushBreadcrumb(breadcrumb, cruBreadcrumbData);
                },
                beforeDataReport: function (data) {
                    return wxBeforeDataReport(data, customTag);
                },
                backTrackerId: backTrackerId,
                trackDsn: 'https://trycatch-end.qa.91jkys.com/errors/upload',
                appOnShow: appOnShow,
    ...
    Object.assign(wx, { mitoLog: log, trackPage: trackPage, trackEvent: trackEvent });
}
